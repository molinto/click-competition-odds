...
def get_soup(
    ...
    for attempt in range(1, retries + 1):
        ...
        if r.status_code == 403:
            print(f"403 Forbidden ({attempt}/{retries}): {url}")
            time.sleep(10 * attempt)
            continue

        if r.status_code >= 500:
            print(f"{r.status_code} Server error ({attempt}/{retries})")
...
<body>
  <div class="wrap">
    ...
    <div class="footer">
      Generated at {generated_at} by Sharry
    </div>
  </div>
...
def main() -> None:
    ...
    for (cat_name, cat_url) in args.category:
        ...
        if not cat_soup:
            print("  !! Skipping category due to repeated 403s")
            continue
